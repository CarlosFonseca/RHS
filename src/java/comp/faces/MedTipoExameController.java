...
import comp.session.MedTipoExameFacade;
...
@ManagedBean(name = "medTipoExameController")
@SessionScoped
public class MedTipoExameController {

    private MedTipoExame current;
    private LazyDataModel items = null;
    @EJB
    private comp.session.MedTipoExameFacade ejbFacade;
    private int selectedItemIndex;
    private int tableWidth;

    public MedTipoExameController() {
    }

    public MedTipoExame getSelected() {
        if (current == null) {
            current = new MedTipoExame();
            selectedItemIndex = -1;
        }
        return current;
    }

    private MedTipoExameFacade getFacade() {
        return ejbFacade;
    }
    ...
    public String prepareView() {
        current = (MedTipoExame) getItems().getRowData();
    ...
    @FacesConverter(forClass = MedTipoExame.class)
    public static class MedTipoExameControllerConverter implements Converter {

        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            if (value == null || value.length() == 0) {
                return null;
            }
            MedTipoExameController controller = (MedTipoExameController) facesContext.getApplication().getELResolver().
                    getValue(facesContext.getELContext(), null, "medTipoExameController");
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof MedTipoExame) {
                MedTipoExame o = (MedTipoExame) object;
                return getStringKey(o.getId());
            } else {
                throw new IllegalArgumentException("object " + object + " is of type " + object.getClass().getName() + "; expected type: " + MedTipoExameController.class.getName());
            }
        }
    }
}