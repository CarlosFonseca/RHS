...
import comp.session.MedResultadoFacade;
...
@ManagedBean(name = "medResultadoController")
@SessionScoped
public class MedResultadoController {

    private MedResultado current;
    private LazyDataModel items = null;
    @EJB
    private comp.session.MedResultadoFacade ejbFacade;
    private int selectedItemIndex;
    private int tableWidth;

    public MedResultadoController() {
    }

    public MedResultado getSelected() {
        if (current == null) {
            current = new MedResultado();
            selectedItemIndex = -1;
        }
        return current;
    }

    private MedResultadoFacade getFacade() {
        return ejbFacade;
    }
    ...
    public String prepareView() {
        current = (MedResultado) getItems().getRowData();
    ...
    @FacesConverter(forClass = MedResultado.class)
    public static class MedResultadoControllerConverter implements Converter {

        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            if (value == null || value.length() == 0) {
                return null;
            }
            MedResultadoController controller = (MedResultadoController) facesContext.getApplication().getELResolver().
                    getValue(facesContext.getELContext(), null, "medResultadoController");
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof MedResultado) {
                MedResultado o = (MedResultado) object;
                return getStringKey(o.getId());
            } else {
                throw new IllegalArgumentException("object " + object + " is of type " + object.getClass().getName() + "; expected type: " + MedResultadoController.class.getName());
            }
        }
    }
}